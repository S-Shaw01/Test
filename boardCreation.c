...
void createBoard(int boardSize, struct slot **upLeft, struct slot **upRight, struct slot **downLeft, struct slot **downRight){
	...
	*downRight = &board[boardSize -1][boardSize -1];
	...
	int i,c,j;
		...
		{
			for(c=0; c < boardSize; i++)
			{
				j = 1+rand()%3;//chooses random number between 1=3
				if(j == 1)
				{
					strcpy(board[i][c].TypeofSlot,"Ground");// if 1, type = ground
				}
				if(j == 2)
				{
					strcpy(board[i][c].TypeofSlot,"City");	//if 2, type = city
				}
				if(j == 3)
				{
					strcpy(board[i][c].TypeofSlot,"Hill");//if 3, type = hill
				}
			}
